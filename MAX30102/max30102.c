...
{
	IIC_Write_Byte(MAX30102_Device_address,REG_INTR_ENABLE_1,0xc0);//// INTR setting使能
	IIC_Write_Byte(MAX30102_Device_address,REG_INTR_ENABLE_2,0x02);//
	IIC_Write_Byte(MAX30102_Device_address,REG_FIFO_WR_PTR,0x00);//FIFO_WR_PTR[4:0]缓存
	...
	IIC_Write_Byte(MAX30102_Device_address,REG_FIFO_CONFIG,0x0f);//sample avg = 1平均样本, fifo rollover=false, fifo almost full = 17 有十七个样本未读时发出中断
	IIC_Write_Byte(MAX30102_Device_address,REG_MODE_CONFIG,0x03);//0x02 for Red only, 0x03 for SpO2 mode 0x07 multimode LED 设置模式 SpO2 和 Red and IR
	...
	IIC_Write_Byte(MAX30102_Device_address,REG_TEMP_CONFIG,0x01); //温度
}
...
/**
  * @brief	读取max30102的温度寄存器
  * @param	无
  * @retval 温度
  */
float MAX30102_read_temp(void)
{
	uint8_t Temp_Integer,Temp_Frac; //整数 小数
	...
	while(MAX30102_INTPin_Read() == 0)
	{
		...
		Temp_Integer = IIC_Read_Byte(MAX30102_Device_address,REG_TEMP_INTR);
		Temp_Frac    = IIC_Read_Byte(MAX30102_Device_address,REG_TEMP_FRAC);
	
	}
	return Temp = Temp_Integer + (float)Temp_Frac*0.0625;
}
