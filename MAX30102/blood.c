...
void blood_Loop(void)
{
    ...
    OLED_ShowNum(1, 13, g_blooddata.heart, 3); // 心率

    // UsartPrintf(USART_DEBUG,"指令血氧%0.2f",g_blooddata.SpO2);
    OLED_ShowNum(2, 13, g_blooddata.SpO2, 3); // 血氧

    if (g_blooddata.SpO2 == 0) {
        UsartPrintf(USART_DEBUG, "未检测到人体！/r/n");
    } else {
        if (isnan(g_blooddata.SpO2))
            g_blooddata.SpO2 = 0;
        UsartPrintf(USART_DEBUG, "心率:%d\r\n", g_blooddata.heart);
        UsartPrintf(USART_DEBUG, "血氧:%f\r\n", g_blooddata.SpO2);

        Data_heart = g_blooddata.heart;
        Data_spo2  = g_blooddata.SpO2;
    }
}
