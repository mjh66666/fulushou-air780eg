...
/**
 * @brief  读取mpu6050的加速度和角速度
 * @param	数据
 * @retval 无
 */
void MPU6050_Get_accel_Data(float *acc_x, float *acc_y, float *acc_z)
{
    short data_acc[3];

    mpu_get_accel_reg(data_acc, NULL);

    *acc_x = ((float)data_acc[0] / 32767) * 19.6; // 量程为+-2000
    *acc_y = ((float)data_acc[1] / 32767) * 19.6;
    *acc_z = ((float)data_acc[2] / 32767) * 19.6;
}
 ...
 * @retval
 */
float MPU6050_SMA(int n)
{
    float Data_SMA[3], SMA;
    int i;
    float acc_x, acc_y, acc_z;
    for (i = 0; i < n; i++) {
        MPU6050_Get_accel_Data(&acc_x, &acc_y, &acc_z);
        Data_SMA[0] = acc_x;
        Data_SMA[1] = acc_y;
        Data_SMA[2] = acc_z;

        SMA += Data_SMA[0] + Data_SMA[1] + Data_SMA[2];
    }
    return SMA /= n;
}

float MPU6050_SMV(int n)
{
    float Data_SMV[3], SMV;
    int i;
    float acc_x, acc_y, acc_z;
    for (i = 0; i < n; i++) {
        MPU6050_Get_accel_Data(&acc_x, &acc_y, &acc_z);
        ...
        SMV += sqrt(Data_SMV[0] * Data_SMV[0] + Data_SMV[1] * Data_SMV[1] + Data_SMV[2] * Data_SMV[2]);
    }
    return SMV /= n;
}
