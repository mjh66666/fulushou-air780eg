...
void MPU6050_Init(void)
{
    ...
    mpu6050_write_reg(PWR_MGMT_1, 0X01);   // 唤醒MPU6050 x轴陀螺作为参考
    mpu6050_write_reg(SMPLRT_DIV, 0x07);   // 陀螺仪采样率125Hz（07）  50hz（13）
    mpu6050_write_reg(CONFIG, 0x06);       // 低通滤波频率5Hz（06）	20hz(04)
    mpu6050_write_reg(GYRO_CONFIG, 0x18);  // 陀螺仪自检及测量范围:不自检，2000deg/s
    mpu6050_write_reg(ACCEL_CONFIG, 0x00); // 加速计自检
}
